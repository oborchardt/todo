...
func main() {
	...
	mux.Handle("DELETE /todos", middlewares.AuthenticateUser(http.HandlerFunc(controllers.DeleteTodo)))
	// according to https://stackoverflow.com/questions/28459418/use-of-put-vs-patch-methods-in-rest-api-real-life-scenarios
	mux.Handle("PATCH /todos", middlewares.AuthenticateUser(http.HandlerFunc(controllers.UpdateTodo)))
	mux.Handle("POST /share", middlewares.AuthenticateUser(http.HandlerFunc(controllers.ShareTodo)))
	mux.Handle("DELETE /share", middlewares.AuthenticateUser(http.HandlerFunc(controllers.UnshareTodo)))
